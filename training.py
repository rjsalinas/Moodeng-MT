#!/usr/bin/env python3
"""
Training v1: mBART50 + LoRA derived from model_training_baseline.py with v1 hyperparameters.

Configuration:
- Dataset: 5k records with 70/15/15 split (3.5k train, 750 val, 750 test)
- LoRA r=64, alpha=128, dropout=0.1; target attn and feed-forward modules
- Standard batch size without gradient accumulation
- AdamW with higher LR (5e-5) and linear schedule
- Evaluate BLEU/chrF every epoch (200 samples for speed, full at end)
- Best model saved to mbart50-finetuned-LoRA-best
"""
...
def main():
    ...
    print("🚀 Starting mBART50 + LoRA training (v1)")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Evaluate (limit to 200 samples for better reliability)
        print("📊 Evaluating...")
        val_metrics = evaluate_model(model, val_dataloader, tokenizer, device, max_samples=200)
        # Compute validation loss on same budget
        val_loss = compute_validation_loss(model, val_dataloader, criterion, device, max_samples=200)
    ...
    log_file = f"training_logs/training_v1_{timestamp}.json"
